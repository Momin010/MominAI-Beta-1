            ...
            <nav className="flex-1 flex flex-col gap-2">
                {navItems.map(item => (
                    <button 
                        ...
                        className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-200 border border-transparent glass-button ${activeView === item.id ? 'bg-[var(--accent)] text-white shadow-lg' : 'text-gray-300 hover:bg-white/20 hover:text-white'}`}
                    >
...
const ProjectsView: React.FC<{onOpenIDE: () => void}> = ({ onOpenIDE }) => {
    const [projects, setProjects] = useState([
        {
            id: '1',
            name: 'My Web App',
            description: 'A production-ready web application generated by MominAI.',
            type: 'web',
    ...
    const handleCreateNewProject = () => {
        ...
        const newProject = {
            ...
            name: 'New Project',
            description: 'A new project created with MominAI.',
            type: 'web',
            updatedAt: 'Just now',
            status: 'draft',
    ...
    const handleOpenProject = (projectId: string) => {
        // Here you would load the specific project
        ...
        <div>
            <div className="flex justify-between items-center mb-8">
                ...
                <button
                    ...
                    className="flex items-center gap-2 glass-button bg-primary text-white px-4 py-2 font-semibold text-sm hover:bg-primary/80 border-none transition-all duration-200 hover:scale-105"
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {projects.map((project, index) => (
                    <div
                        key={project.id}
                        className="glass-card flex flex-col gap-4 group cursor-pointer hover:scale-105 transition-all duration-200"
                    ...
                    >
                        ...
                        <p className="text-sm text-[var(--gray)] flex-1">{project.description}</p>
                        <div className="flex justify-between items-center text-xs text-[var(--gray)]">
                            ...
                            <span className="flex items-center gap-1.5">
                                <div className={`w-2 h-2 rounded-full ${
                                    ...
                                    project.status === 'draft' ? 'bg-yellow-400' : 'bg-gray-400'
                                }`}></div>
                                {project.status === 'deployed' ? 'Deployed' :
                                 project.status === 'draft' ? 'Draft' : 'Building'}
                            </span>
                        </div>
                        <button
                            onClick={(e) => {
                            ...
                            className="w-full mt-2 glass-button text-gray-300 px-4 py-2.5 text-sm font-semibold hover:bg-white/20 border border-white/30 transition-all duration-200"
                        >
                            Open IDE
                        </button>
                    </div>
                ...
                <div
                    className="glass-card border-2 border-dashed border-white/30 flex items-center justify-center text-center cursor-pointer hover:border-primary/50 hover:bg-white/10 transition-all duration-300 hover:scale-105"
                    style={{ animation: 'fade-in-up 0.5s ease-out 0.3s backwards' }}
                    onClick={handleCreateNewProject}
                >
                    <div>
                        ...
                        <p className="text-xs text-[var(--gray)] mt-1">Start building with AI</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div>
            ...
            <div className="flex gap-8 items-start">
                <div className="w-48 flex-shrink-0 flex flex-col gap-2">
                   {tabs.map(tab => (
                        <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`w-full flex items-center gap-3 p-3 rounded-lg text-sm font-medium transition-all duration-200 glass-button border-none ${activeTab === tab.id ? 'bg-primary/20 text-white shadow-lg' : 'text-gray-300 hover:bg-white/20'}`}>
    ...
    <div>
        ...
        <form className="flex flex-col gap-6 max-w-lg">
             <div className="flex items-center gap-4">
                 ...
                 <button className="glass-button border border-white/30 px-4 py-2 text-sm font-semibold text-gray-300 hover:bg-white/20">Upload new picture</button>
             </div>
             <div className="grid grid-cols-2 gap-4">
                <div>
                    <label className="text-sm text-[var(--gray)] mb-1 block">First Name</label>
                    <input type="text" value="Momin" className="w-full p-2 glass-input text-gray-200" />
                </div>
                 <div>
                    <label className="text-sm text-[var(--gray)] mb-1 block">Last Name</label>
                    <input type="text" value="Khan" className="w-full p-2 glass-input text-gray-200" />
                </div>
            </div>
             <div>
                <label className="text-sm text-[var(--gray)] mb-1 block">Email</label>
                <input type="email" value="momin@example.com" disabled className="w-full p-2 glass-input opacity-60 cursor-not-allowed text-gray-200" />
            </div>
...
const ApiKeysSettings = () => (
     <div>
        <div className="flex justify-between items-center mb-6">
            ...
            <button className="flex items-center gap-2 glass-button text-gray-300 px-4 py-2 font-semibold text-sm hover:bg-white/20 border border-white/30">
                <AppIcons.Plus className="w-5 h-5" />
